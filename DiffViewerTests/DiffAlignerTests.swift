...
struct DiffAlignerTests {
    @Test func whitespaceOnlyChangeHiddenAlignsAsEqual() {
        let rows = DiffAligner.align(oldLines: ["x = 1", "y"], newLines: ["x  =  1", "y"], hideWhitespace: true, hints: DifftHints())
    ...
    @Test func pureAdditionProducesPadRows() {
        let rows = DiffAligner.align(oldLines: ["a", "c"], newLines: ["a", "b", "c"], hideWhitespace: true, hints: DifftHints())
    ...
    @Test func replacementZipsLinesSideBySide() {
        let rows = DiffAligner.align(oldLines: ["a", "b", "c"], newLines: ["x", "y"], hideWhitespace: true, hints: DifftHints())
    ...
    @Test func difftPairsDriveAlignmentWithinBlock() {
        ...
        let rows = DiffAligner.align(oldLines: ["p", "q"], newLines: ["r", "s", "t"], hideWhitespace: true, hints: hints)

...
final class WindowState {
        ...
        case let .success(newFiles):
            ...
            session.statsTask = Task { [weak self] in
                await self?.attachLineStats(
                    to: newFiles, session: session, serial: serial, ignoreWhitespace: ignoreWhitespace)
            }
    ...
    private func attachLineStats(
        to newFiles: [ChangedFile], session: RepoSession, serial: Int, ignoreWhitespace: Bool
    ) async {
        let client = session.client
